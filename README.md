...
Rust + Tauri 桌面版 Bing Copilot（New Bing / Sydney）本地重制客户端，深度还原 2024 版 Bing Copilot 界面与交互。

纯本地单机运行，无登录、不自托管对外服务。支持两种对话链路：

| 链路 | 说明 |
|---|---|
| **本地自定义 OpenAI 兼容接口** | 在「设置 → AI 接口设置」中填写 `baseURL / apiKey / model`，走 `fetch + SSE` 流式对话，支持 `reasoning_content` 思考过程 |
| **本地 New Bing（Sydney）代理** | 通过 Rust 内核直连 `wss://sydney.bing.com/sydney/ChatHub`，使用 Cookie（`BING_HEADER` / `_U`）鉴权与伪造 IP |

界面已在「深色模式」下同步适配：右上角主题切换按钮、设置图标随主题变色，底部页脚栏与背景融为一体，并已隐藏右侧滚动条。

> 本项目为非官方重制项目，仅供学习与娱乐，与 Microsoft / Bing 无任何关联。
...
```bash
npm install
npm run tauri dev     # 启动 Tauri 桌面开发（自动拉起 Vite：http://localhost:1420）
```
...
- 💬 Bing Copilot 深度还原 UI（2024 版）：输入框、回复卡片、引用徽标、底部页脚栏
- 🖼️ 图片上传：文件选择器、复制图片后直接粘贴上传
- 🎤 语音对话：语音输入与 TTS 播报
- 🎨 对话风格切换：创意 / 平衡 / 精确，深浅色主题与各样式背景联动
- 📋 历史记录：本地持久化会话，随时回到之前的对话
- ⚙️ 提示词管理、自定义系统提示词、高级模型参数、Partial Mode
...
> 复制出来的内容应该如下所示。确认格式无误后，在你的应用「设置 → AI 接口设置 → 用户信息」中粘贴进去，点击“转成 BING_HEADER 并复制”，然后从剪切板粘贴即可得到。
