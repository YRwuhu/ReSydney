      ...
      <div className="attribution-container">
        <div className="attribution-items">
          {sourceAttributions.map((attribution, index) => {
            ...
            // 防御：URL 可能是相对地址或畸形，解析失败时降级为显示标题，避免整条消息渲染崩溃
            let host = ''
            try {
              host = new URL(/^https?:\/\//i.test(seeMoreUrl) ? seeMoreUrl : `https://${seeMoreUrl}`).host
            } catch {
              host = ''
            }
            const label = host || providerDisplayName
            return (
              <a
                key={index}
                ...
                target="_blank"
                role="listitem"
                href={/^https?:\/\//i.test(seeMoreUrl) ? seeMoreUrl : providerDisplayName.length ? undefined : '#'}
