...
export default function Chat({ className }: ChatProps) {
  ...
  const onExpaned = useCallback((flag: boolean) => {
    setExpand(flag)
  }, [])
  ...
  return (
    <div className={cn(bingStyle.toLowerCase(), { 'side-panel-expanded': expand })}>
      ...
      <ChatHistory expand={expand} onToggle={onExpaned} />
      ...
      <div className="flex justify-center left-0 w-full">
        <div className={cn('main-root items-center flex-1 pb-16', className)}>
          <div className="main-content">
            ...
            {messages.length ? (
              <>
                <ChatList messages={messages} generatingMessageId={generatingMessageId} />
                ...
                {generating ? (
                  <div className="flex h-10 items-center justify-center my-4">
                    ...
                    >
                      <StopIcon width={24} className="mr-1" fill="var(--cib-color-foreground-accent-primary)" />
                      <span>停止响应</span>
                    </button>
                  </div>
                ) : null}
